...
const constructAxes = (canvas, trendsData, financeData) => {
	...
	yScaleLeft = yScaleLeftFn(trendsData);
	console.log('yScaleLeft done');
	yScaleRight = yScaleRightFn(financeData);
	console.log('yScaleRight done');
...
// const populateGoogleTrendsData = (nodes, canvas) => {
	...
	// 	.y0(height-padding) //  big number
	// 	.y1(function(d) { return yScaleLeft(+d.value[0]); }); // big num max, min min
...
const populateGoogleTrendsData = (nodes, canvas) => {
	...
	nodes[0].value.forEach((searchItem, index) => { // just doing this x number of times
		if (index > 0) {
			console.log(index);
...
const populateFinanceData = (nodes, canvas) => {
	...
	canvas.selectAll('.line')
	   ...
	   .attr('y1', function(d) { return yScaleRight(d.source.value) })
	   .attr('x2', function(d) { return xScale(new Date(d.target.time*1000)); })
	   .attr('y2', function(d) { return yScaleRight(d.target.value) })
...
const constructGraph = (node, trendsData, financeData) => {
	...
	canvas.attr('width', width)
		.attr('height', height);
		console.log('before constructAxes');
